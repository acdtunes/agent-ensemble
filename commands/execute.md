...
argument-hint: '[agent] [feature-id] [step-id] - Example: @en-software-crafter "auth-upgrade" "01-01"'
---

# EN-EXECUTE: Atomic Task Execution
...
```
/en:execute @{agent} "{feature-id}" "{step-id}"
```
...
Before dispatching the agent, read rigor config from `.en/des-config.json` (key: `rigor`). If absent, use standard defaults.
...
2. Read rigor profile from `.en/des-config.json` (default: standard)
...
```bash
/en:execute @en-software-crafter "des-us007-boundary-rules" "02-01"
/en:execute @en-researcher "auth-upgrade" "01-01"
/en:execute @en-software-crafter "des-us007" "03-01"  # retry after failure
```
...
**Handoff To**: /en:review for post-execution review
