...
**Wave**: CROSS_WAVE (entry point) | **Agent**: Main Instance (self — orchestrator) | **Command**: `/en:fast-forward`

## Overview

Chains remaining waves end-to-end after single user confirmation. Detects current progress (like `/en:continue`), shows planned sequence, runs each wave automatically — DISCUSS > DESIGN > DEVOPS > DISTILL > DELIVER — without stopping between waves.
...
**New project (description provided):** Derive feature ID per `skills/common/wizard-shared-rules.md` (Feature ID Derivation). Show derived ID, allow override via AskUserQuestion. Create `docs/feature/{feature-id}/`.

**Existing project (no description):** Scan `docs/feature/` (same as `/en:continue` Step 1-2). If multiple, ask user to select.

### Step 3: Detect Current Progress

Check wave artifacts using Wave Detection Rules in `skills/common/wizard-shared-rules.md`.

### Step 4: Determine Wave Sequence

Default order (DISCOVER skipped): DISCUSS > DESIGN > DEVOPS > DISTILL > DELIVER

**With `--from` flag:** Validate prerequisite artifacts exist. If missing: "Cannot start from {wave} — {missing} artifacts missing. Run `/en:continue` to fill the gap." Start from specified wave.
...
2. Show error:
   ...
   ✗ {WAVE} failed
     Error: [details]
     Progress saved. Run /en:continue to resume from {WAVE}.
   ```
3. Suggest `/en:continue` for manual resume
...
| Error | Response |
|-------|----------|
| No description and no existing projects | Suggest `/en:new` |
| `--from` with missing prerequisites | List missing artifacts, refuse |
| Wave failure mid-pipeline | Stop, show error, suggest `/en:continue` |
| Artifact verification fails after wave | Treat as wave failure |
| Name conflict on new project | Same as `/en:new` conflict handling |
...
- [ ] Failure stops pipeline with clear error and `/en:continue` suggestion
...
```
/en:fast-forward "Upgrade authentication to OAuth2"
```
...
```
/en:fast-forward --from=distill rate-limiting
```
...
```
/en:fast-forward "Add payment processing"
```
DISCUSS succeeds, DESIGN succeeds, DEVOPS fails. Pipeline stops. Shows error and suggests `/en:continue` to resume from DEVOPS.
