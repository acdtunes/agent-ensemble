...
# EN-DISTILL: Acceptance Test Creation and Business Validation

**Wave**: DISTILL (wave 5 of 6) | **Agent**: Quinn (en-acceptance-designer)
...
Before dispatching the acceptance designer, read rigor config from `.en/des-config.json` (key: `rigor`). If absent, use standard defaults.

- **`agent_model`**: Pass as `model` parameter to Task tool. If `"inherit"`, omit `model` (inherits from session).

## Agent Invocation

@en-acceptance-designer

Execute /en:create-acceptance-tests for {feature-id}.
...
- [ ] Handoff package ready for en-software-crafter (DELIVER wave)

## Examples

### Example 1: Core feature acceptance tests
```
/en:distill payment-webhook --test-framework=pytest-bdd --integration=real-services
```
...
**Handoff To**: en-software-crafter (DELIVER wave)
