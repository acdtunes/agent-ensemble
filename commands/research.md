...
# EN-RESEARCH: Evidence-Driven Knowledge Research

**Wave**: CROSS_WAVE
**Agent**: Nova (en-researcher)
**Command**: `*research`
...
1. **Read** `.en/trusted-source-domains.yaml` from the project root
2. **If file missing**, seed it from the defaults in the `## Default Trusted Sources` section below, then notify the user:
   "Seeded `.en/trusted-source-domains.yaml` with defaults (7 categories, 42 trusted domains, 5 excluded). Edit the YAML directly to customize."
...
Execute /en:research on {topic} [--skill-for={agent-name}].
...
- skill_output_directory: skills/{agent-name}/
...
- [ ] Skill file created in skills/{agent-name}/
...
```
/en:research "event sourcing patterns" --research_depth=detailed
```
...
```
/en:research "mutation testing methodologies" --skill-for=software-crafter
```
Nova researches mutation testing, distills into practitioner-focused skill file at skills/software-crafter/.

## Expected Outputs

```
docs/research/{category}/{topic}-comprehensive-research.md
skills/{agent}/{topic}-methodology.md    (if --skill-for)
```

## Default Trusted Sources

The following YAML is the default content for `.en/trusted-source-domains.yaml`. It is seeded automatically when the file does not exist.
