...
**Wave**: DESIGN (wave 3 of 6) | **Agents**: Morgan (en-solution-architect) | **Command**: `*design-architecture`
...
- FP: `This project follows the **functional programming** paradigm. Use @en-functional-software-crafter for implementation.`
- OOP: `This project follows the **object-oriented** paradigm. Use @en-software-crafter for implementation.`
...
Before dispatching the architect agent, read rigor config from `.en/des-config.json` (key: `rigor`). If absent, use standard defaults.
...
Execute /en:design-architecture for {feature-id}.
...
**SKILL_LOADING**: Read your skill files at `skills/solution-architect/`. At Phase 4, always load: `architecture-patterns.md`, `architectural-styles-tradeoffs.md`. Do NOT load `roadmap-design.md` during DESIGN wave -- roadmap creation belongs to the DELIVER wave (`/en:roadmap` or `/en:deliver`). Then follow your Skill Loading Strategy table for phase-specific skills.
...
- [ ] Handoff accepted by en-platform-architect (DEVOPS wave)

## Next Wave

**Handoff To**: en-platform-architect (DEVOPS wave)
