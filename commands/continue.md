...
# EN-CONTINUE: Resume a Feature

**Wave**: CROSS_WAVE (entry point) | **Agent**: Main Instance (self — wizard) | **Command**: `/en:continue`
...
**No directories found:** Display "No active projects found under `docs/feature/`." Suggest `/en:new`. Stop.
...
Check each wave's artifacts using Wave Detection Rules in `skills/common/wizard-shared-rules.md`.
...
| Error | Response |
|-------|----------|
| No `docs/feature/` directory | Suggest `/en:new` |
| Empty project directory | Suggest `/en:new` or re-run from DISCUSS |
...
/en:continue
```
Wizard finds one project: `notification-service`. DISCUSS artifacts exist (complete), no DESIGN artifacts. Shows progress, recommends DESIGN. User confirms, wizard launches `/en:design notification-service`.

### Example 2: DELIVER resume
```
/en:continue rate-limiting
```
Wizard checks `rate-limiting` project. All waves through DISTILL complete, DELIVER in progress (steps 01-01 through 02-01 done). Shows "Next: step 02-02", launches `/en:deliver "rate-limiting"`.
...
/en:continue
```
Wizard finds no `docs/feature/` directories. Shows "No active projects found" and suggests `/en:new`.
