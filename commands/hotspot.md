...
# EN-HOTSPOT: Code Crime Scene Hotspot Lens
...
```
/en:hotspot --rank code-smell-detector-report.md
```
...
```
/en:hotspot --detail src/services/payment.ts
```
...
Run `/en:hotspot --top=10 --json` first, then pass the file list to:
...
Run `/en:hotspot --rank {report.md}` after any analysis to prioritize by churn.

### In nWave workflows

- Before `/en:refactor` — identify which files to refactor first
- Before `/en:review` — focus review effort on high-churn areas
- Before `/en:root-why` — check if the problematic area is a known hotspot
...
```
/en:hotspot --rank code-refactoring-report.md
```
