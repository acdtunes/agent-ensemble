...
Usage:
    python -m en.cli.parallel_groups analyze ROADMAP_PATH
...
from en.adapters.roadmap_adapter import load_roadmap
from en.cli.team_state import _get_step_deps
...
def extract_steps(roadmap: dict) -> list[Step]:
    ...
    for phase in roadmap.get("phases", []):
        ...
        for step in phase.get("steps", []):
            ...
            blocked_by = _get_step_deps(step)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv:
        print("Usage: python -m en.cli.parallel_groups analyze ROADMAP_PATH")
