...
**How**: Use the Read tool to load files from `skills/agent-builder/`
...
**How**: Use the Read tool to load files from `skills/{agent-name}/`
...
12. **Mandatory skill loading section** with imperative language ("You MUST load", not "you should load") and explicit path `skills/{agent-name}/`
...
| Missing skills path | Sub-agents can't find skills without explicit path | Document `skills/{agent-name}/` in agent |
...
4. Move TDD phase details to en-software-crafter agent -- 200 lines saved
...
Copy skill files from `skills/{agent-b}/` to `skills/{agent-a}/`|If agent B has reviewer, copy its skills to `skills/{agent-a-reviewer}/`|Update surviving agent's and reviewer's frontmatter skill references

### Phase 4: Clean Up (mandatory — do not skip)
Delete deprecated agent file: `agents/en-{agent-b}.md`|Delete deprecated reviewer if exists: `agents/en-{agent-b}-reviewer.md`|Delete deprecated skill directories: `skills/{agent-b}/`, `skills/{agent-b}-reviewer/`|Delete deprecated command task files (e.g., `nWave/tasks/nw/{command}.md`)
